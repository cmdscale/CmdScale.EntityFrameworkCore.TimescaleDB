...
{
    public class TimescaleDbSqlGenerationTests() : MigrationsSqlGeneratorTestBase(
            TimescaleTestHelpers.Instance,
            new ServiceCollection().AddEntityFrameworkNpgsqlNetTopologySuite(),
            TimescaleTestHelpers.Instance.AddProviderOptions(
                    ((IRelationalDbContextOptionsBuilderInfrastructure)
                        new NpgsqlDbContextOptionsBuilder(new DbContextOptionsBuilder())
                            .UseNetTopologySuite())
                    .OptionsBuilder).Options)
    {
        ...
        public override void DefaultValue_with_line_breaks(bool isUnicode)
        {
            ...
            CREATE TABLE dbo."TestLineBreaks" (
                "TestDefaultValue" text NOT NULL DEFAULT '
            Various Line
            Breaks
            '
            );
        ...
        public override void DefaultValue_with_line_breaks_2(bool isUnicode)
        {
            ...
            string defaultValue = string.Join(string.Empty, Enumerable.Range(0, 300).Select(e => e.ToString() + "\r\n"));

            AssertSql(
            $"""
            CREATE TABLE dbo."TestLineBreaks" (
                "TestDefaultValue" text NOT NULL DEFAULT '{defaultValue.Replace("'", "''")}'
            );
            """);
        }
        ...
        public override void Sequence_restart_operation(long? startsAt)
        {
            ...
            string expectedSql = startsAt.HasValue
                ? $"""
                ALTER SEQUENCE dbo."TestRestartSequenceOperation" START WITH {startsAt.Value};
        ...
        protected new void AssertSql(string expectedSql)
        {
            Assert.Equal(expectedSql.TrimEnd(), Sql.TrimEnd(), ignoreLineEndingDifferences: true);
        }
    }
}